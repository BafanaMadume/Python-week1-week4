...
* Python errors and exceptions ultimately stem from a class called the base exception ,for example the division by zero error is refered to as a **arithmetic error**= a type of exception ,which in turn extends the base exception class.
* Base Exception class provides usefu; and powerul properties to exceptions ,such as halting code execution and providing information about why and how the execution was halted
* A **stack trace** is a behind-the-scenes report generated by a computer program when an error or exception occurs. It acts like a roadmap, showing the exact sequence of function calls (or method calls in object-oriented programming) that led to the issue.


**Call Stack**: Imagine a stack of plates. Each plate represents a function call. As your program runs, it keeps adding new function calls (plates) on top of the stack. When an error happens, the stack trace captures a snapshot of this stack, listing the functions involved from the most recent call (top plate) down to the initial call (bottom plate).
**Error Location**: By examining the stack trace, you can pinpoint the exact line of code where the error originated within a specific function. This is incredibly valuable for debugging purposes, as it helps you identify the root cause of the problem.
Why are stack traces important?

**Debuggers' Best Friend**: Programmers rely on stack traces to diagnose errors efficiently. The trace reveals the chain of events that led to the issue, making it easier to fix the problematic code.
Identifying the Culprit: Stack traces can differentiate between errors caused by user input or external factors and those originating from within the program itself.
Understanding Program Flow: By analyzing stack traces, developers can gain insights into how their program functions, particularly the sequence of function calls and potential bottleneck
